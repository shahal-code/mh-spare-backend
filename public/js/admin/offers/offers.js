async function toggleOfferStatus(id, isActive, name) {
    const action = isActive ? 'Deactivate' : 'Activate';
    const color = isActive ? '#f43f5e' : '#10b981';

    const result = await Swal.fire({
        title: `<span class="text-white">${action} Offer?</span>`,
        text: `Are you sure you want to ${action.toLowerCase()} the offer "${name}"?`,
        icon: 'warning',
        background: '#111827',
        color: '#fff',
    ...
    try {
        ...
        if (data.success) {
            Swal.fire({
                icon: 'success',
                title: 'Updated!',
                text: data.message || `Offer has been ${action.toLowerCase()}d.`,
                background: '#111827',
                color: '#fff',
...
async function deleteOffer(id) {
    const result = await Swal.fire({
        title: '<span class="text-white">Delete Offer?</span>',
    ...
    try {
        ...
        if (data.success) {
            Swal.fire({
                icon: 'success', title: 'Deleted!', text: data.message || 'Offer has been deleted.',
